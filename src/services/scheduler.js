...
async function refreshPendingShipments(origin = 'agendado') {
  ...
  if (totalCpfs === 0) {
    await Shipment.saveSchedulerLog({ total_cpfs: 0, updated: 0, promoted: 0, errors: 0, pending_after: 0, origin });
    return;
  }

  let updated = 0, promoted = 0, errors = 0, ticketsClosed = 0;
  const statusBreakdown = {}; // { [status]: count }
  ...
  for (const [cpf, { shipments, queued }] of Object.entries(cpfMap)) {
    try {
      ...
      for (const shipment of shipments) {
        ...
        // Contabiliza breakdown por status
        const st = tracking.status || 'unknown';
        statusBreakdown[st] = (statusBreakdown[st] || 0) + 1;

        // Auto-fecha tickets de "sem movimentação" quando movimentação detectada
        const MOVEMENT_STATUSES = ['forwarded','delivering','delivered','posted_object','recipient_not_found','returning','returned','waiting_client','wrong_address','delivery_problem'];
        if (MOVEMENT_STATUSES.includes(tracking.status) && shipment.status !== tracking.status) {
          ...
          if (openTickets?.length) {
            await db.from('tickets')
              .update({ status: 'Concluído', closed_at: new Date().toISOString() })
              .in('id', openTickets.map(t => t.id));
            ticketsClosed += openTickets.length;
            console.log(`[H7] Auto-fechou ${openTickets.length} ticket(s) de Envio para ${shipment.tracking_code} — movimentação detectada`);
          }
        }
      }
      ...
      if (queued.length && tracking.loggi_code) {
        ...
        for (const q of queued) {
          ...
          // Fecha tickets de "sem rastreio" em aberto para este pedido
          const { data: openRastrTickets } = await db
            .from('tickets')
            .select('id')
            .eq('order_id', q.order_id)
            .in('status', ['Aberto', 'Em andamento']);
          if (openRastrTickets?.length) {
            await db.from('tickets')
              .update({ status: 'Concluído', closed_at: new Date().toISOString() })
              .in('id', openRastrTickets.map(t => t.id));
            ticketsClosed += openRastrTickets.length;
            console.log(`[H7] Auto-fechou ${openRastrTickets.length} ticket(s) para order ${q.order_id} — rastreio encontrado`);
          }
        }
      }
  ...
  await Shipment.saveSchedulerLog({
    total_cpfs: totalCpfs, updated, promoted, errors, pending_after,
    started_at: startedAt, finished_at: new Date().toISOString(), origin,
    status_breakdown: statusBreakdown, tickets_closed: ticketsClosed,
  });
  console.log(`[Scheduler] Concluído: ${updated} atualizados, ${promoted} promovidos, ${errors} erros, ${pending_after} ainda ativos, ${ticketsClosed} tickets fechados`);
}
