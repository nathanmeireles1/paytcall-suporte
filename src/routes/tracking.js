...
router.get('/:code', async (req, res) => {
  ...
  try {
    ...
    const updatedShipment = await Shipment.upsert({
      ...shipment,
      tracking_code: code,
    ...
    await Shipment.saveSchedulerLog({
      total_cpfs: 1, updated: 1, promoted: 0, errors: 0, pending_after: 0,
      started_at: startedAt, finished_at: new Date().toISOString(), origin: 'manual',
    });

    res.json({ ...updatedShipment, events: await Shipment.getEvents(code) });
  } catch (err) {
    await Shipment.saveSchedulerLog({
      total_cpfs: 1, updated: 0, promoted: 0, errors: 1, pending_after: 0,
      started_at: startedAt, finished_at: new Date().toISOString(), origin: 'manual',
    }).catch(() => {});
...
router.post('/refresh', async (req, res) => {
  ...
  refreshPendingShipments('manual').catch(console.error);
});
...
router.post('/refresh-batch', async (req, res) => {
  ...
  (async () => {
    ...
    await Shipment.saveSchedulerLog({
      total_cpfs: codes.length, updated, promoted: 0, errors, pending_after: 0,
      started_at: startedAt, finished_at: new Date().toISOString(), origin: 'manual',
    });
  })().catch(console.error);
});
