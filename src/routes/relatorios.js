...
router.get('/tickets', requirePermission('relatorio_tickets', 'can_view'), async (req, res) => {
  try {
    ...
    const forcedAssignedTo = isRestricted ? req.user.name : null;

    // Forçar filtro de tipo para roles específicos
    const forcedTipo = req.user.role === 'logistica' ? 'LOGISTICA'
                     : req.user.role === 'retencao'  ? 'RETENCAO'
                     : null;

    const result = await Shipment.getAllTickets({
      tipo:        forcedTipo || tipo,
      status,
...
router.get('/tickets/export', requirePermission('relatorio_tickets', 'can_view'), async (req, res) => {
  try {
    ...
    const { tipo, status, assigned_to, priority, motivo, search, format = 'xlsx' } = req.query;
    const result = await Shipment.getAllTickets({ tipo, status, assigned_to, priority, motivo, search, page: 1, limit: 9999 });
